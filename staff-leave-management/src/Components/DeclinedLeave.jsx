...
const DeclinedLeave = () => {
  const [declinedLeaveRequests, setdeclinedLeaveRequests] = useState([]);

  useEffect(() => {
    // Fetch pending leave requests when the component mounts
    fetchdeclinedLeaveRequests();
  }, []);

  const fetchdeclinedLeaveRequests = () => {
    // Make an API call to fetch pending leave requests
    // You can replace the URL with your actual API endpoint
    fetch('http://localhost:3001/api/declinedLeaveRequests')
      .then((response) => response.json())
      .then((data) => setdeclinedLeaveRequests(data))
      .catch((error) => console.error('Error fetching declined leave requests:', error));
  };
      ...
      <div className="table-container" style={{ overflowY: 'auto', maxHeight: '500px' }}>
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableBody>
              {declinedLeaveRequests.map((request, index) => (
                <TableRow
